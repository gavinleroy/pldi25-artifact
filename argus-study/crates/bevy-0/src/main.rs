...
use bevy::{
  color::palettes::basic::YELLOW, prelude::*,
};
...
fn setup(
  ...
  commands.spawn((
    PbrBundle {
      mesh: meshes.add(
        Sphere::new(3.0).mesh().ico(32).unwrap(),
      ),
      material: materials
        .add(Color::from(YELLOW)),
...
fn rotate_cube(
  ...
  for (mut transform, cube) in &mut cubes {
    ...
    let look_at_sphere = transform
      .looking_at(center, *transform.local_y());
    ...
    let incremental_turn_weight =
      cube.turn_speed * timer.delta_seconds();
    let old_rotation = transform.rotation;
    transform.rotation = old_rotation.lerp(
      look_at_sphere.rotation,
      incremental_turn_weight,
    );
  }
}
...
fn scale_down_sphere_proportional_to_cube_travel_distance(
  ...
  for (cube_transform, cube_state) in &cubes {
    distances += (cube_state.start_pos
  ...
  for (mut transform, center) in &mut centers {
    ...
    let mut new_size: f32 = center.max_size
      - center.scale_factor * distances;
