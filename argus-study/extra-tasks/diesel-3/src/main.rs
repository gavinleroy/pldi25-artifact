...
fn m_to_n_relations(
  ...
  let books =
    BookAuthor::belonging_to(&astrid_lindgren)
      .inner_join(books::table)
      .select(Book::as_select())
      .load(conn)?;
  ...
  let authors =
    BookAuthor::belonging_to(&collaboration)
      .inner_join(authors::table)
      .select(Author::as_select())
      .load(conn)?;
  println!(
    "Authors for \"Pipi and Momo\": {authors:?}"
  );
  ...
  let books_per_author: Vec<(Author, Vec<Book>)> =
    books
...
fn setup_data(
  ...
  new_page(
    conn,
    2,
    "den prachtvollen Theatern...",
    momo.id,
  )?;

  // create an author
  let michael_ende =
    new_author(conn, "Michael Ende")?;
  ...
  let astrid_lindgren =
    new_author(conn, "Astrid Lindgren")?;
