...
const SALT_STRING: &str =
  "dec7901d02ee422ba6bd0333e4fef137";
...
impl From<argon2::password_hash::Error>
...
pub use self::AuthenticationError::{
  IncorrectPassword, NoPasswordSet, NoUsernameSet,
};

#[derive(
  Queryable, Identifiable, Debug, PartialEq, Eq,
)]
pub struct User {
  pub id: i32,
  pub username: String,
...
fn find_user(
  ...
  if let Some(user_and_password) =
    user_and_password
  {
    let parsed_hash = PasswordHash::new(
      &user_and_password.password,
    )?;
    Argon2::default()
...
fn if_not_present<T>(
  ...
  res.map_err(|e| match e {
    dotenvy::Error::EnvVar(NotPresent) => {
      on_not_present
    }
...
mod tests {
  ...
  fn current_user_returns_the_user_if_it_has_the_same_password(
    ...
    let expected_user =
      register_user(conn, "sgrif", "hunter2")
        .unwrap();
    let user =
      find_user(conn, "sgrif", "hunter2")
        .unwrap();
  ...
  fn current_user_fails_if_password_does_not_match(
    ...
    register_user(conn, "sgrif", "letmein")
      .unwrap();
    let result =
      find_user(conn, "sgrif", "hunter2");
