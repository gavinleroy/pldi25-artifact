...
fn run_cli(
  ...
  match cli.command {
    Command::AllPosts { page, per_page } => {
      ...
      let (posts_with_user, total_pages) = query
        .load_and_count_pages::<(Post, User)>(
          conn,
        )?;
      let (posts, post_users): (Vec<_>, Vec<_>) =
        posts_with_user.into_iter().unzip();
    ...
    Command::CreatePost { title } => {
      ...
      let id = diesel::insert_into(posts::table)
        ...
        .get_result::<i32>(conn)?;
      println!(
        "Successfully created post with id {id}"
      );
    }
    Command::EditPost { post_id, publish } => {
      ...
      let post = Post::belonging_to(&user)
        ...
        .first::<Post>(conn)?;
      let new_body =
        editor::edit_string(&post.body)?;

      let updated_status = match post.status {
        Draft if publish => {
          Some(published_at.eq(now.nullable()))
        }
        _ => None,
      };
    ...
    Command::AddComment {
      ...
      let inserted =
        diesel::insert_into(comments)
          ...
          .get_result::<i32>(conn)?;
      println!(
        "Created comment with ID {inserted}"
      );
    }
    Command::EditComment { comment_id } => {
      ...
      diesel::update(comments)
        .set(body.eq(editor::edit_string(
          &comment.body,
        )?))
        .execute(conn)?;
    }
    Command::MyComments { page, per_page } => {
      ...
      let (comments_and_post_title, total_pages) = query
        .load_and_count_pages::<(Comment, String)>(conn)?;
...
) -> Result<auth::User, Box<dyn Error>> {
...
fn register_user(
  ...
  match auth::register_user_from_env(conn) {
    Ok(_) => Ok(()),
    Err(Auth::DatabaseError(DatabaseError(
      UniqueViolation,
      _,
    ))) => Err(
